...
func TestFilter(t *testing.T) {
	...
	logger, buffer = log.MakeTestLogger(zap.WrapCore(func(c zapcore.Core) zapcore.Core {
		return log.NewFilterCore(c, "github.com/tikv/migration/br/").With([]zap.Field{zap.String("a", "b")})
	}), zap.AddCaller())
	logger.Warn("the message", zap.Int("number", 123456), zap.Ints("array", []int{7, 8, 9}))
	...
	logger.Warn("the message", zap.String("stack", "github.com/tikv/migration/br/"))
	require.Len(t, buffer.Stripped(), 0)
}
...
//
// Run `go test -run='^$' -bench=. -v github.com/tikv/migration/br/pkg/lightning/log` to get benchmark result.
func BenchmarkFilterStringsContains(b *testing.B) {
	...
	inputs := []string{
		...
		"github.com/tikv/migration/br/some/package/path",
	}
