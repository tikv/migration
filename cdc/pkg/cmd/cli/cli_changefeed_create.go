...
func (o *createChangefeedOptions) completeCfg(ctx context.Context, cmd *cobra.Command) error {
	...
	if len(o.commonChangefeedOptions.configFile) > 0 {
		if err := o.commonChangefeedOptions.strictDecodeConfig("TiKVCDC changefeed", cfg); err != nil {
			return err
		}
	}
	...
	if !cdcClusterVer.ShouldEnableOldValueByDefault() {
		...
		log.Warn("The TiKVCDC cluster is built from an older version, disabling old value by default.",
