...
func (b *mockFlushTaskBuilder) generateRowChanges(tsRangeBegin, tsRangeEnd uint64, count int) *mockFlushTaskBuilder {
	...
	for fTs := float64(tsRangeBegin); fTs < float64(tsRangeEnd); fTs += density {
		ts := int(fTs)
		kvEntry := generateMockRawKV(0, ts)[0]
...
func (b *mockFlushTaskBuilder) addResolved(ts uint64) *mockFlushTaskBuilder {
	_ = b.writer.writeNext(model.NewResolvedPolymorphicEvent(0, ts, 0))
...
func (s *sorterSuite) TestMergerSortDelay(c *check.C) {
	...
	err = failpoint.Enable("github.com/tikv/migration/cdc/cdc/sorter/unified/sorterMergeDelay", "sleep(5)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/tikv/migration/cdc/cdc/sorter/unified/sorterMergeDelay")
	}()
...
func (s *sorterSuite) TestMergerCancel(c *check.C) {
	...
	// enable the failpoint to simulate delays
	err = failpoint.Enable("github.com/tikv/migration/cdc/cdc/sorter/unified/sorterMergeDelay", "sleep(10)")
	c.Assert(err, check.IsNil)
	defer func() {
		_ = failpoint.Disable("github.com/tikv/migration/cdc/cdc/sorter/unified/sorterMergeDelay")
	}()
...
func (s *sorterSuite) TestMergerOutputBlocked(c *check.C) {
	...
	err := failpoint.Enable("github.com/tikv/migration/cdc/cdc/sorter/unified/sorterDebug", "return(true)")
	c.Assert(err, check.IsNil)
	defer failpoint.Disable("github.com/tikv/migration/cdc/cdc/sorter/unified/sorterDebug") //nolint:errcheck
