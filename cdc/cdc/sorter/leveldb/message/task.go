...
type IterRequest struct {
	...
	IterCh chan *LimitedIterator `json:"-"` // Make Task JSON prinkeyspan.
}
...
func (k Key) String() string {
	uid, keyspanID, startTs, CRTs := encoding.DecodeKey([]byte(k))
	return fmt.Sprintf(
		"uid: %d, keyspanID: %d, startTs: %d, CRTs: %d",
		uid, keyspanID, startTs, CRTs)
}
...
// NewCleanupTask returns a clean up task to clean up keyspan data.
func NewCleanupTask(uid uint32, keyspanID uint64) Task {
	return Task{
		KeySpanID: keyspanID,
		UID:       uid,
		Cleanup:   true,
