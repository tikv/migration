...
func TestEncodeKey(t *testing.T) {
	t.Parallel()
	mustLess := func(uniqueIDa, uniqueIDb uint32, keyspanIDa, keyspanIDb uint64, a, b *model.PolymorphicEvent) {
		keya, keyb := EncodeKey(uniqueIDa, keyspanIDa, a), EncodeKey(uniqueIDb, keyspanIDb, b)
...
func TestEncodeTsKey(t *testing.T) {
	t.Parallel()
	mustLess := func(uniqueIDa, uniqueIDb uint32, keyspanIDa, keyspanIDb uint64, a *model.PolymorphicEvent, b uint64) {
		keya, keyb := EncodeKey(uniqueIDa, keyspanIDa, a), EncodeTsKey(uniqueIDb, keyspanIDb, b)
		require.Equal(t, bytes.Compare(keya, keyb), -1)
		require.Equal(t, len(keya), cap(keya))
		require.Equal(t, len(keyb), cap(keyb))
	}
...
func TestDecodeKey(t *testing.T) {
	...
	uid, keyspanID, startTs, CRTs := DecodeKey(key)
	require.EqualValues(t, 1, uid)
	require.EqualValues(t, 2, keyspanID)
