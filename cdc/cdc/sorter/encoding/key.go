...
// DecodeKey decodes a key to uniqueID, keyspanID, startTs, CRTs.
func DecodeKey(key []byte) (uniqueID uint32, keyspanID uint64, startTs, CRTs uint64) {
	// uniqueID, keyspanID, CRTs, startTs, Key, Put/Delete
	// uniqueID
	uniqueID = binary.BigEndian.Uint32(key)
	// keyspan ID
	keyspanID = binary.BigEndian.Uint64(key[4:])
	// CRTs
...
// EncodeTsKey encodes uniqueID, keyspanID, CRTs.
func EncodeTsKey(uniqueID uint32, keyspanID uint64, ts uint64) []byte {
	// uniqueID, keyspanID, CRTs.
	...
	// keyspanID
	binary.BigEndian.PutUint64(uint64Buf[:], keyspanID)
...
// Format: uniqueID, keyspanID, CRTs, startTs, Put/Delete, Key.
func EncodeKey(uniqueID uint32, keyspanID uint64, event *model.PolymorphicEvent) []byte {
	if event.RawKV == nil {
		log.Panic("rawkv must not be nil", zap.Any("event", event))
	}
	// uniqueID, keyspanID, CRTs, startTs, Put/Delete, Key
